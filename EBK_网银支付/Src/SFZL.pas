...
type
  TFormSFZL = class(TForm)
    ...
    procedure ClientDataSetSFZLGridAfterScroll(DataSet: TDataSet);
...
begin
    //添加没有授权不可查阅
    if not TPower.GetPower(GN_SFZL_Read) then
    begin
      SysMessage('没有查阅权限。', AOther_JG, [mbOk]);
      Exit;
    end;
    if Application.FindComponent('FormSFZL')=nil then
        Application.CreateForm(TFormSFZL,FormSFZL);
    FormSFZL.Show;
end;


procedure TFormSFZL.SpeedBarDblClick(Sender: TObject);
begin
    PAllowButtonCaption(Speedbar);
end;

procedure TFormSFZL.SpeedItemClick(Sender: TObject);
var
    TreeNode:TTreeNode ;
    szBS:String ;
begin
   ...
   //修改
    ...
    //保存
        ...
        ClientDataSetJCZLGrid.Locate('provid',szBS,[]) ;
    //取消
    end
...
function TFormSFZL.CheckData: Boolean;
var
    szBS,szMC : String;
begin
    Result := False ;
...
function TFormSFZL.SaveData: Boolean;
var
    szSQL:String;
    szBS,szMC :String ;
begin
    ...
    if (not (Length(EditDM.Text)=2))  then begin
      ShowMessage('省份代码请输入两位');
      Exit;
    end;
    if iAddOrEdit=0 then begin  //新增保存
        szSQL := ' Insert into ebk_province( provid,province) values ( '
               + '''' + szBS      + ''', '
               + '''' + szMC      + ''')' ;
    end else begin              //修改保存
        szSQL :=  ' Update ebk_province set '
               + '  province=''' + szMC   + ''' '
               + ' Where provid=''' + szBS + ''' ' ;
    end;
...
function TFormSFZL.AskIfDel: Boolean;
begin
...
//判断是否已经使用
function TFormSFZL.isUsed: Boolean;
var szSQL,szDM: string;
begin
    szDM  := Trim(EditDM.Text);
    szSQL := 'select count(*) as num from ebk_areacode where provid='''+szDM+'''';
...
//删除数据
Function TFormSFZL.FDeleteData(szTableName,szFieldName,szFieldValue:String;szWhere:String=''):Boolean ;
var
    szSQL: String ;
begin
...
Function  TFormSFZL.DeleteData:Boolean;
var
    szBS:String ;
begin
    Result := False ;
    ...
    if isUsed then begin  //判断是否已使用了
      SysMessage('该省份资料已使用，不能删除！','_YB',[mbOk]);
      Exit ;
    end;
    if not FDeleteData('ebk_province','provid',Trim(szBS)) then Exit ;
    Result := True ;
end;

procedure TFormSFZL.ShowData;
begin
    With ClientDataSetJCZLGrid do begin
        EditDM.Text  := FieldByName('provid').asstring ;
        EditMC.Text  := FieldByName('province').asstring ;
    end;
end;

procedure TFormSFZL.RefreshData;
var
    szSQL:string;
begin
    szSQL := 'select * from ebk_province order By provid ' ;
    OpenDBTree(szSQL,'provid','province','2');
    OpenDBGrid(szSQL);
end;


procedure TFormSFZL.FormClose(Sender: TObject;
...
procedure TFormSFZL.SpeedItemAddClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemAdd);
end;

procedure TFormSFZL.SpeedItemCancelClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemCancel);
end;

procedure TFormSFZL.SpeedItemDeleteClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemDelete);
end;

procedure TFormSFZL.SpeedItemEditClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemEdit);
end;

procedure TFormSFZL.SpeedItemExitClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemExit);
end;

procedure TFormSFZL.SpeedItemFilterClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemFilter);
end;

procedure TFormSFZL.SpeedItemFindClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemFind);
end;

procedure TFormSFZL.SpeedItemHelpClick(Sender: TObject);
begin
    ChangeHelp('EBK.hlp',32);
end;

procedure TFormSFZL.SpeedItemNoFilterClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemNoFilter);
end;

procedure TFormSFZL.SpeedItemPreviewClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemPreview);
end;

procedure TFormSFZL.SpeedItemPrintClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemPrint);
end;

procedure TFormSFZL.SpeedItemRefreshClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemRefresh);
end;

procedure TFormSFZL.SpeedItemSaveClick(Sender: TObject);
begin
    SpeedItemClick(SpeedItemSave);
end;

procedure TFormSFZL.ClientDataSetSFZLGridAfterScroll(DataSet: TDataSet);
begin
    SpeedItemClick(N_Cancel);
end;

procedure TFormSFZL.EditMCKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
    ACtrl:TWinControl;
begin
...
procedure TFormSFZL.OpenDBGrid(szSQL:string);
begin
    //打开表
    if szSQL = '' then  Exit;
...
procedure TFormSFZL.OpenDBTree(szSQL, szField, szFieldName,szRule : string);
begin
    //打开树
...
procedure TFormSFZL.THDBGridJCZLHeaderDblClick(ACol: Integer;
  var CanLock: Boolean; AShift: TShiftState);
begin
    CanLock := True;
end;

procedure TFormSFZL.DBTreeJCZLChange(Sender: TObject; Node: TTreeNode);
var
    szTemp:string;
begin
    if blneedSaved then
...
procedure TFormSFZL.FormCreate(Sender: TObject);
begin
    ...
    SetTHFilter;
    SetFinderField('provid','province');
    PGetFieldLength(EditDM,'ebk_province','provid');
    PGetFieldLength(EditMC,'ebk_province','province');
end;

procedure TFormSFZL.SetTHFilter;
begin
...
procedure TFormSFZL.SetBtnEnabled;
var
    i:integer;
begin
    ...
    //添加没有授权编辑功能不可用
    if not TPower.GetPower(GN_SFZL_Edit) then
    begin
      SetGNFalse;
    end;
end;
...
procedure TFormSFZL.SetBtnDisabled;
var
    i:integer;
begin
    for i:=0 to self.ComponentCount-1 do
...
procedure TFormSFZL.SetBtnStatus(Sender: TObject);
begin
...
procedure TFormSFZL.FormShow(Sender: TObject);
begin
...
procedure TFormSFZL.FinderJCZLFound(Sender: TObject);
var
    szTemp:string;
begin
    try
...
procedure TFormSFZL.SetFinderField(szCode, szName: string);
begin
...
//返回字段长度
Procedure  TFormSFZL.PGetFieldLength(Sender:Tobject;szTableName,szFieldName:String);
begin
    with DataModulePub.ClientDataSetTmp do
    begin
        try
...
procedure TFormSFZL.THFilterJCZLFilter(Sender: TObject;
  AFilter: String);
begin
...
procedure TFormSFZL.PSetNoFilterBtnStatus;
begin
    N_NoFilter.Enabled := (szCtrlFilter<>'') ;
    SpeedItemNoFilter.Enabled := (szCtrlFilter<>'') ;
end;

procedure TFormSFZL.EditPXHKeyPress(Sender: TObject; var Key: Char);
begin
...
//设置控制状态
procedure TFormSFZL.PSetStateOfContrl(szLX:string);
var
    i:Integer ;
begin
...
procedure TFormSFZL.EditMCChange(Sender: TObject);
var
    VStrPy:string;
begin
    //助记码最多允许10位
...
procedure TFormSFZL.SetGNFalse;
begin
...
procedure TFormSFZL.EditDMKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFormSFZL.FormCloseQuery(Sender: TObject;
...
//执行无返回值的数据操作
function TFormSFZL.ExeEditSql(dbTmp:TClientDataSet;szSQL:String):Boolean;
var
    i:Integer ;
begin
...
//执行有返回值的数据操作
function TFormSFZL.ExeQuerySql(dbTmp:TClientDataSet;szSQL:String):Boolean;
var
    i:Integer ;
begin
    with dbTmp do
...
//获得焦点
Procedure TFormSFZL.PSetFocus(Sender:TWinControl) ;
begin
  try
...
//检查输入的字符是否合法，是否是0－9；A-Z;a-z
Function  TFormSFZL.CheckInStr(InStr:string):Boolean ;
var
    i,iCharOrd:integer;
begin
    Result:=False;
...
procedure TFormSFZL.EditDMExit(Sender: TObject);
begin
  if (not (Length(EditDM.Text)=2))  then begin
    ShowMessage('省份代码请输入两位');
    Exit;
