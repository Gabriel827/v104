...
type
  ...
  TPLATSpeedButton = class(TGraphicControl)
  private
    FGroupIndex: Integer;
    FGlyph: Pointer;
    FDown: Boolean;
...
function GeTPLATBitbtnGlyph(Kind: TBitbtnKind): TBitmap;
begin
...
constructor TPLATSpeedButton.Create(AOwner: TComponent);
begin
...
destructor TPLATSpeedButton.Destroy;
begin
  Dec(ButtonCount);
  inherited Destroy;
...
procedure TPLATSpeedButton.Paint;
const
...
procedure TPLATSpeedButton.UpdateTracking;
var
  P: TPoint;
begin
  if FFlat then
...
procedure TPLATSpeedButton.Loaded;
var
  State: TButtonState;
begin
  inherited Loaded;
...
procedure TPLATSpeedButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TPLATSpeedButton.MouseMove(Shift: TShiftState; X, Y: Integer);
var
...
(*
    ...
    self.Left := self.Left + X - FDragPosX;
    self.Top := self.Top + Y - FDragPosY;
  end
  else if not FMouseInControl then
    UpdateTracking;
end;

procedure TPLATSpeedButton.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DoClick: Boolean;
begin
...
procedure TPLATSpeedButton.Click;
begin
  inherited Click;
end;

function TPLATSpeedButton.GetPalette: HPALETTE;
begin
  Result := Glyph.Palette;
end;

function TPLATSpeedButton.GetGlyph: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).Glyph;
end;

procedure TPLATSpeedButton.SetGlyph(Value: TBitmap);
begin
  TButtonGlyph(FGlyph).Glyph := Value;
  Invalidate;
end;

function TPLATSpeedButton.GetNumGlyphs: TNumGlyphs;
begin
  Result := TButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TPLATSpeedButton.SetNumGlyphs(Value: TNumGlyphs);
begin
  if Value < 0 then
    Value := 1
...
procedure TPLATSpeedButton.GlyphChanged(Sender: TObject);
begin
  Invalidate;
end;

procedure TPLATSpeedButton.UpdateExclusive;
var
  Msg: TMessage;
begin
...
procedure TPLATSpeedButton.SetDown(Value: Boolean);
begin
  if FGroupIndex = 0 then
    Value := False;
  if Value <> FDown then
...
procedure TPLATSpeedButton.SetFlat(Value: Boolean);
begin
  if Value <> FFlat then
  begin
    FFlat := Value;
...
procedure TPLATSpeedButton.SetGroupIndex(Value: Integer);
begin
...
procedure TPLATSpeedButton.SetLayout(Value: TButtonLayout);
begin
...
procedure TPLATSpeedButton.SetMargin(Value: Integer);
begin
...
procedure TPLATSpeedButton.SetSpacing(Value: Integer);
begin
...
procedure TPLATSpeedButton.SetTransparent(Value: Boolean);
begin
...
procedure TPLATSpeedButton.SetAllowAllUp(Value: Boolean);
begin
...
procedure TPLATSpeedButton.WMLButtonDblClk(var Message: TWMLButtonDown);
begin
  inherited;
  if FDown then
    DblClick;
...
procedure TPLATSpeedButton.CMEnabledChanged(var Message: TMessage);
const
...
procedure TPLATSpeedButton.CMButtonPressed(var Message: TMessage);
var
  Sender: TPLATSpeedButton;
begin
  if Message.WParam = FGroupIndex then
  begin
    Sender := TPLATSpeedButton(Message.LParam);
    if Sender <> Self then
    begin
...
procedure TPLATSpeedButton.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
...
procedure TPLATSpeedButton.CMFontChanged(var Message: TMessage);
begin
  Invalidate;
end;

procedure TPLATSpeedButton.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
end;

procedure TPLATSpeedButton.CMSysColorChange(var Message: TMessage);
begin
...
procedure TPLATSpeedButton.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TPLATSpeedButton.CMMouseLeave(var Message: TMessage);
begin
  inherited;
...
procedure TPLATSpeedButton.ActionChange(Sender: TObject; CheckDefaults: Boolean);
...
constructor TPLATBitbtn.Create(AOwner: TComponent);
begin
  FGlyph := TButtonGlyph.Create;
  TButtonGlyph(FGlyph).OnChange := GlyphChanged;
  inherited Create(AOwner);
...
destructor TPLATBitbtn.Destroy;
begin
  inherited Destroy;
  TButtonGlyph(FGlyph).Free;
  FCanvas.Free;
...
procedure TPLATBitbtn.CreateHandle;
var
  State: TButtonState;
begin
  if Enabled then
...
procedure TPLATBitbtn.CreateParams(var Params: TCreateParams);
begin
...
procedure TPLATBitbtn.SetButtonStyle(ADefault: Boolean);
begin
...
procedure TPLATBitbtn.CNMeasureItem(var Message: TWMMeasureItem);
begin
...
procedure TPLATBitbtn.CNDrawItem(var Message: TWMDrawItem);
begin
  DrawItem(Message.DrawItemStruct^);
end;

procedure TPLATBitbtn.DrawItem(const DrawItemStruct: TDrawItemStruct);
var
...
procedure TPLATBitbtn.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TPLATBitbtn.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TPLATBitbtn.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  Perform(WM_LBUTTONDOWN, Message.Keys, Longint(Message.Pos));
end;

function TPLATBitbtn.GetPalette: HPALETTE;
begin
  Result := Glyph.Palette;
end;

procedure TPLATBitbtn.SetGlyph(Value: TBitmap);
begin
...
function TPLATBitbtn.GetGlyph: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).Glyph;
end;

procedure TPLATBitbtn.GlyphChanged(Sender: TObject);
begin
  Invalidate;
end;

function TPLATBitbtn.IsCustom: Boolean;
begin
  Result := Kind = bkCustom;
end;

procedure TPLATBitbtn.SetStyle(Value: TButtonStyle);
begin
...
procedure TPLATBitbtn.SetKind(Value: TBitbtnKind);
begin
  if Value <> FKind then
  begin
    ...
    begin
      ...
      TButtonGlyph(FGlyph).Glyph := GeTPLATBitbtnGlyph(Value);
      NumGlyphs := 2;
...
function TPLATBitbtn.IsCustomCaption: Boolean;
begin
 // Result := AnsiCompareStr(Caption, LoadResString(BitBtnCaptions[FKind])) <> 0;
end;

function TPLATBitbtn.GetKind: TBitbtnKind;
begin
...
procedure TPLATBitbtn.SetLayout(Value: TButtonLayout);
begin
  if FLayout <> Value then
  begin
    FLayout := Value;
...
function TPLATBitbtn.GetNumGlyphs: TNumGlyphs;
begin
  Result := TButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TPLATBitbtn.SetNumGlyphs(Value: TNumGlyphs);
begin
  if Value < 0 then
    Value := 1
  else if Value > 4 then
...
procedure TPLATBitbtn.SetSpacing(Value: Integer);
begin
...
procedure TPLATBitbtn.SetMargin(Value: Integer);
begin
  if (Value <> FMargin) and (Value >= -1) then
  begin
    FMargin := Value;
...
procedure TPLATBitbtn.ActionChange(Sender: TObject; CheckDefaults: Boolean);
